...
# 4. 학습 결과 및 평가 (y: 실제값, y_pred: 모델의 과거 데이터 예측값)
y_pred = model.predict(X)
...
print(f"==== 모델 성능 평가 (Regression Metrics) ==== ")
print(f"1) 결정계수 (R2 Score): {r2:.4f}")
print(f"   -> 1에 가까울수록 모델이 전체 흐름을 잘 설명함")
print("-" * 40)
print(f"2) 평균 절대 오차 (MAE): ${mae:.2f}")
print(f"   -> 모델이 평균적으로 약 {mae:.2f}달러 정도 틀리고 있음 (가장 직관적)")
print("-" * 40)
print(f"3) 평균 제곱 오차 (MSE): {mse:.2f}")
print(f"4) 평균 제곱근 오차 (RMSE): ${rmse:.2f}")
print(f"   -> 큰 오차에 더 민감하게 반응하는 지표")
