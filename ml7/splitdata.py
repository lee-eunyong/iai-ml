...
def split_train_test_data(hiwy, output_dir):
    ...
    # 출력
    print("\nStep 4: Raw Data Splitting Results (6:2:2)")
    print("-" * 50)
    ...
    # ⭐ CSV 저장 (UTF-8 권장)
    train_set.to_csv(output_dir / "oil_value_train.csv", index=False, encoding="utf-8-sig")
    valid_set.to_csv(output_dir / "oil_value_valid.csv", index=False, encoding="utf-8-sig")
    test_set.to_csv(output_dir / "oil_value_test.csv", index=False, encoding="utf-8-sig")

    print("\nStep 5: Saving Files Complete")
    print("-" * 50)
